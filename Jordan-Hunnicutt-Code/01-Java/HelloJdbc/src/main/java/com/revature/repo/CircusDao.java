...
public class CircusDao implements DaoContract<Circus, String>{
	...
	public Circus findById(String i) {
		...
		try (Connection conn = EnvironmentConnectionUtil.getInstance().getConnection()){
			String sql = "select * from circus where name = ?"; //really, you should never use simple statements
	...
	public Circus create(Circus t) {
		try(Connection conn = EnvironmentConnectionUtil.getInstance().getConnection()){
			String sql = "insert into circus (name, ringleader, is_entertaining) values (?, ?, ?)";
			PreparedStatement ps = conn.prepareStatement(sql);
