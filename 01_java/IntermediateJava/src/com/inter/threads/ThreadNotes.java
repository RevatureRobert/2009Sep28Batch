 ...
 * 			importance of the thread's process. 10 is the highest while 1
 ...
 * 						scheduler.
 * 
 * 			Blocked - waiting on a synchronized resource to become available to 
 * 						perform its operation
 * 
 * 			Waiting - waiting for another thread to complete execution for an 
 * 						undetermined amount of time
 * 
 * 			TimedWaiting - waiting for another thread to complete execution for a
 ...
 * 
 * 			Deadlock 	- 	A situation where 2 threads have holds on resources while
 * 								one thread needs the others's resource to complete its
 * 								operation, and the same goes for the other thread to 
 * 								let go og it's. Since neither thread can gain access
 * 								to the resource it needs, they are in a deadlock.
 * 
 * 			Producer consumer problem	-	One thread produces data, the other consumes the information.
 * 												When they are out of sync and working faster than the other,
 * 												this creates data overflow or data starvation.
 * 
 * 		user threads vs daemon threads
 * 
 * 			user threads	-	threads that we create with the purpose of joining the main thread
 * 
 * 			daemon thread	-	background threads whose purpose is outside the main threads concern.
 ...
 * 			sleep(long milliseconds) - causes the currently executing thread to temporarily stop for a 
