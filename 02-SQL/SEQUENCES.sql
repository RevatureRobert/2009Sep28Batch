------sequences
--a sequence will begin a sequence of numbers and keep track of the current
----value to use the values, we can use nextval('seq_name') to get a unique
----value.
...
create function primary_key_gen() returns trigger as $$
begin
	if(new.id=0) then
